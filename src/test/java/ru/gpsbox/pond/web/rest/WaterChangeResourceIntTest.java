...
import ru.gpsbox.pond.repository.WaterChangeRepository;
import ru.gpsbox.pond.repository.search.WaterChangeSearchRepository;
import ru.gpsbox.pond.service.dto.WaterChangeDTO;
import ru.gpsbox.pond.service.mapper.WaterChangeMapper;
...
public class WaterChangeResourceIntTest {
     ...
     */
    public static WaterChange createEntity() {
    ...
    public void initTest() {
        waterChangeRepository.deleteAll();
        waterChangeSearchRepository.deleteAll();
        waterChange = createEntity();
    }
    ...
    public void getAllWaterChanges() throws Exception {
        ...
        restWaterChangeMockMvc.perform(get("/api/water-changes?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(waterChange.getId())))
    ...
    public void getWaterChange() throws Exception {
        ...
        restWaterChangeMockMvc.perform(get("/api/water-changes/{id}", waterChange.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.id").value(waterChange.getId()))
