...
import ru.gpsbox.pond.repository.TankRepository;
import ru.gpsbox.pond.repository.search.TankSearchRepository;
import ru.gpsbox.pond.service.dto.TankDTO;
import ru.gpsbox.pond.service.mapper.TankMapper;
...
public class TankResourceIntTest {
     ...
     */
    public static Tank createEntity() {
        Tank tank = new Tank()
    ...
    public void getAllTanks() throws Exception {
        ...
        restTankMockMvc.perform(get("/api/tanks?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(tank.getId())))
    ...
    public void getTank() throws Exception {
        ...
        restTankMockMvc.perform(get("/api/tanks/{id}", tank.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.id").value(tank.getId()))
