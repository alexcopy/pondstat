...
export class MeterReadingMySuffixService {
    ...
    private resourceUrl =  SERVER_API_URL + 'api/meter-readings';
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/meter-readings';
    ...
    search(req?: any): Observable<HttpResponse<MeterReadingMySuffix[]>> {
        const options = createRequestOption(req);
        return this.http.get<MeterReadingMySuffix[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<MeterReadingMySuffix[]>) => this.convertArrayResponse(res));
    }
