...
export class WaterChangeMySuffixService {
    ...
    private resourceUrl =  SERVER_API_URL + 'api/water-changes';
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/water-changes';
    ...
    search(req?: any): Observable<HttpResponse<WaterChangeMySuffix[]>> {
        const options = createRequestOption(req);
        return this.http.get<WaterChangeMySuffix[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<WaterChangeMySuffix[]>) => this.convertArrayResponse(res));
    }
