...
export class LiveStockMySuffixService {
    ...
    private resourceUrl =  SERVER_API_URL + 'api/live-stocks';
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/live-stocks';
    ...
    search(req?: any): Observable<HttpResponse<LiveStockMySuffix[]>> {
        const options = createRequestOption(req);
        return this.http.get<LiveStockMySuffix[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<LiveStockMySuffix[]>) => this.convertArrayResponse(res));
    }
