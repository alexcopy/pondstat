...
export class ChemicalsMySuffixService {
    ...
    private resourceUrl =  SERVER_API_URL + 'api/chemicals';
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/chemicals';
    ...
    search(req?: any): Observable<HttpResponse<ChemicalsMySuffix[]>> {
        const options = createRequestOption(req);
        return this.http.get<ChemicalsMySuffix[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<ChemicalsMySuffix[]>) => this.convertArrayResponse(res));
    }
