...
export class TempMeterMySuffixService {
    ...
    private resourceUrl =  SERVER_API_URL + 'api/temp-meters';
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/temp-meters';
    ...
    search(req?: any): Observable<HttpResponse<TempMeterMySuffix[]>> {
        const options = createRequestOption(req);
        return this.http.get<TempMeterMySuffix[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<TempMeterMySuffix[]>) => this.convertArrayResponse(res));
    }
