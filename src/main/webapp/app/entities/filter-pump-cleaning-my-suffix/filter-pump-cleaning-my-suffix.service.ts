...
export class FilterPumpCleaningMySuffixService {
    ...
    private resourceUrl =  SERVER_API_URL + 'api/filter-pump-cleanings';
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/filter-pump-cleanings';
    ...
    search(req?: any): Observable<HttpResponse<FilterPumpCleaningMySuffix[]>> {
        const options = createRequestOption(req);
        return this.http.get<FilterPumpCleaningMySuffix[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<FilterPumpCleaningMySuffix[]>) => this.convertArrayResponse(res));
    }
