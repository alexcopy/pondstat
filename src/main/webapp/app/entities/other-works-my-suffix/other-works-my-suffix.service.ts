...
export class OtherWorksMySuffixService {
    ...
    private resourceUrl =  SERVER_API_URL + 'api/other-works';
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/other-works';
    ...
    search(req?: any): Observable<HttpResponse<OtherWorksMySuffix[]>> {
        const options = createRequestOption(req);
        return this.http.get<OtherWorksMySuffix[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<OtherWorksMySuffix[]>) => this.convertArrayResponse(res));
    }
